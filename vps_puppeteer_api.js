...
app.post('/gsc-get-tag', async (req, res) => {
    ...
    try {
        ...
        // Kiểm tra xem đã login chưa, nếu chưa báo lỗi
        if (page.url().includes('accounts.google.com')) {
            await browser.close();
            return res.status(401).json({ status: 'fail', message: 'Tài khoản chưa đăng nhập hoặc hết phiên. Hãy chạy /init-profile' });
        }
...
app.post('/gsc-click-verify', async (req, res) => {
    ...
    try {
        ...
        return res.json({ status: 'fail', message: 'Lỗi xác minh từ phía Google.' });
    } catch (error) {
        if (browser) await browser.close(); return res.status(500).json({ error: true, message: error.message });
    }
});
